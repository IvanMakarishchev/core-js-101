...
function getSumBetweenNumbers(n1, n2) {
  return new Array(Math.abs(n1 - n2) + 1)
    .fill().map((e, i) => (n1 + i)).reduce((el, acc) => (acc + el));
}
...
function isTriangle(a, b, c) {
  ...
  return arr.pop() - arr.pop() < arr.pop() || false;
}
...
function doRectanglesOverlap(rect1, rect2) {
  const arr1 = new Array(rect1.height).fill().map((y, iY) => (
    new Array(rect1.width).fill().map((x, iX) => (`${iX + rect1.left}, ${iY + rect1.top}`))
  )).flat();
  const arr2 = new Array(rect2.height).fill().map((y, iY) => (
    new Array(rect2.width).fill().map((x, iX) => (`${iX + rect2.left}, ${iY + rect2.top}`))
  )).flat();
  return arr2.filter((el) => arr1.includes(el)).length > 0;
}
...
function isInsideCircle(circle, point) {
  const distX = Math.abs(point.x - circle.center.x);
  const distY = Math.abs(point.y - circle.center.y);
  return (distX ** 2 + distY ** 2) ** 0.5 < circle.radius;
}
...
function findFirstSingleChar(str) {
  ...
  const set = Array.from(new Set(strArr.map((el) => el)));
  return set.map((el) => { strArr.splice(strArr.indexOf(el), 1, ''); return strArr.includes(el) || el; })
    .filter((el) => el !== true)[0];
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const numArray = [a, b].sort();
  const startFlag = isStartIncluded ? '[' : '(';
  const endFlag = isEndIncluded ? ']' : ')';
  return `${startFlag}${numArray.join(', ')}${endFlag}`;
}
