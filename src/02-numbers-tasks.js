...
function getAverage(...args) {
  return args.reduce((el, acc) => (acc / args.length + el / args.length));
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const sideX = Math.abs(x1) + Math.abs(x2);
  const sideY = Math.abs(y1) + Math.abs(y2);
...
function isPrime(num) {
  ...
  for (n; n <= Math.trunc(num ** 0.5); n += 1) {
    if (num % n === 0) {
      return false;
    }
  }
